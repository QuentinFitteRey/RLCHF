...
from prompt.prompts import prompt_base_avsb, prompt_end_avsb
from prompt.generate_prompt import (
    generate_prompt_llama,
    generate_prompt_gemma,
    generate_prompt_qwen,
)
...
def main():
    ...
    if "llama" in args.model:
        df[["prompt_gen", "gt_answer"]] = df.apply(
            lambda row: generate_prompt_llama(
                row, prompt_base_avsb, prompt_end_avsb, personas, train=False
            ),
            axis=1,
            result_type="expand",
        )
        ...
        df[["prompt_gen", "gt_answer"]] = df.apply(
            lambda row: generate_prompt_gemma(
                row, prompt_base_avsb, prompt_end_avsb, personas, train=False
            ),
            axis=1,
            result_type="expand",
        ...
        df[["prompt_gen", "gt_answer"]] = df.apply(
            lambda row: generate_prompt_qwen(
                row, prompt_base_avsb, prompt_end_avsb, personas, train=False
            ),
            axis=1,
            result_type="expand",
        ...
        raise ValueError("Model not implemented. Publish an issue or a merge request.")
    ...
    df["raw_output"] = [output.outputs[0].text for output in outputs]
    df["answer"] = df["raw_output"].apply(lambda x: hotpot_qa_parser(x))

...
def format_question(
    context: str, question: str, ans0: str, ans1: str, ans2: str, instruction: str
) -> str:
    """
    Formats a question and answer options into a string
    """
    return (
        f"{context} {question}\n(a) {ans0} \n(b) {ans1} \n(c) {ans2}\n\n{instruction}"
    )
...
def parse_model_answer(answer_text):
    """
    Parses the model's answer from the expected format and converts it to ans0, ans1, or ans2.

    Parameters:
        answer_text (str): The raw text output from the model.

    Returns:
        str: The parsed answer in ans0, ans1, or ans2 format.
    """
    # Extract the selected letter from the format "ANSWER: [ANS]"
    match = re.search(r"ANSWER:\s*([a-c])", answer_text.strip(), re.IGNORECASE)

    if not match:
        return "Invalid format"  # Handle unexpected formats

    # Map the letter choice to the correct answer format
    choice_mapping = {"a": 0, "b": 1, "c": 2}
    selected_choice = match.group(1).lower()  # Extract and lowercase the matched choice

    return choice_mapping.get(selected_choice, "Invalid choice")

...
import io
import json
from eval.benchmarks.bbq.utils import *
import random
import ast
...
def generate_intersectional_data(
    cats, vocab, names_vocab, BBQ_DIR_PATH, SAVE_DIR_PATH, unknown_options
):

    for cat in tqdm(cats, desc="Processing Intersectional Categories"):
        frames = pd.read_csv(
            BBQ_DIR_PATH + "/templates/new_templates - %s.csv" % cat, na_filter=False
        )  # get the right template file

        os.makedirs(SAVE_DIR_PATH, exist_ok=True)
        dat_file = io.open(
            # "data/%s.jsonl" % cat, "w"
            os.path.join(SAVE_DIR_PATH, "%s.jsonl" % cat),
            "w",
        ...
        for i in tqdm(range(len(the_frames)), desc=f"Templates in {cat}", leave=False):
            ...
            if len(bias_targets) > 0:
                ...
                targeted_word_list = [
                    x for x in possible_word_list if x in bias_targets
                ]
            else:
            ...
            for j in range(len(word_list)):
                ...
                if has_proper_name:
                    ...
                    if cat == "Race_x_SES":
                        ...
                        other_first_names_diff_race = (
                            other_first_names_diff_race.sample(
                                n=5, replace=False
                            ).reset_index()
                        )  # for downsampling
                        ...
                        other_first_names_same_race = (
                            other_first_names_same_race.sample(
                                n=5, replace=False
                            ).reset_index()
                        )  # for downsampling
                ...
                for k in range(len(new_word_list)):
                    ...
                    if the_frames.Names[i] == "" and cat == "Race_x_SES":
                        ...
                        t_word_gs = (
                            vocab[
                                (vocab.Category == "SES") & (vocab.Info == target_var2)
                            ]
                            .Name.unique()
                            .tolist()
                        )
...
if __name__ == "__main__":
    ...
    BBQ_DIR_PATH = "eval/benchmarks/bbq"
    SAVE_DIR_PATH = os.path.join(DATA_DIR, "processed", "bbq")

    # all possible values of cat
    cats = ["Race_x_gender", "Race_x_SES"]

    # read in vocabulary files
    vocab = pd.read_csv(BBQ_DIR_PATH + "/templates/vocabulary.csv")
    vocab = vocab[vocab.Pilot_include != "No"]
    names_vocab = pd.read_csv(BBQ_DIR_PATH + "/templates/vocabulary_proper_names.csv")
